...
The pinned source, binary and release-tag version is `v20260926-2` (date and positive same-day revision); it does not change automatically on each build. A further release on the same date uses `v20260926-3`. Arch maps this to `pkgver=20260926`, `pkgrel=2` (or `pkgrel=3` for that further release). Keep the source archive and binary version identical to the full tag; a release tag must match the pinned source and Arch recipe. No tag or release is created by building locally.
...
includes the binary, a generic private `/etc/xbkeeper/xbkeeper.toml`, documentation
and [service/timer](packaging/systemd/); it does not enable or start them.
...
timer. It supplies a generic private default configuration, but not host-specific
settings, credentials or production activation approval. The default unit
