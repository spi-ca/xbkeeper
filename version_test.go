...
func TestReleaseVersionContract(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"current", version, version, "20260926", "1", true},
		{"same-day-revision", "v20260926-2", "v20260926-2", "20260926", "2", true},
		...
		{"mismatched-make", version, "v20260925-1", "20260926", "1", false},
		{"mismatched-pkgver", version, version, "20260925", "1", false},
		{"mismatched-pkgrel", version, version, "20260926", "2", false},
	} {
...
func TestReleasePinnedChecksum(t *testing.T) {
	...
	for _, valid := range []bool{true, false} {
		t.Run(fmt.Sprintf("valid=%t", valid), func(t *testing.T) {
			...
			if err := os.WriteFile(filepath.Join(dir, "dist/xbkeeper-v20260926-1.tar.gz"), payload, 0600); err != nil {
				t.Fatal(err)
			}
			...
			cmd.Dir = dir
			cmd.Env = append(os.Environ(), "VERSION=v20260926-1")
